...
import { ETH_TICKER, SPOOKY_ROUTER_ADDR, SFI_TICKER } from '../utils/constants';
/**
 * An API module of Sfi Finance contracts.
 * All contract-interacting domain logic should be defined in here.
 */
export class SfiFinance {
  myAccount: string;
  ...
  constructor(cfg: Configuration) {
    ...
    this.SFI = new ERC20(deployments.sfi.address, provider, 'SFI');
    ...
    // Uniswap V2 Pair
    this.SFIWETH_LP = new Contract(externalTokens['SFI-ETH-LP'][0], IUniswapV2PairABI, provider);
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.SFI, this.TSHARE, this.TBOND, ...Object.values(this.externalTokens)];
    for (const token of tokens) {
      token.connect(this.signer);
    }
    this.SFIWETH_LP = this.SFIWETH_LP.connect(this.signer);
  ...
  async getSfiStat(): Promise<TokenStat> {
    const { SfiEthRewardPool, SfiEthLpSfiRewardPool } = this.contracts;
    const supply = await this.SFI.totalSupply();
    const sfiRewardPoolSupply = await this.SFI.balanceOf(SfiEthRewardPool.address);
    const sfiEthLpSfiRewardPoolSupply = await this.SFI.balanceOf(SfiEthLpSfiRewardPool.address);
    const sfiCirculatingSupply = supply
      .sub(sfiRewardPoolSupply)
      .sub(sfiEthLpSfiRewardPoolSupply)
    const priceInETH = await this.getTokenPriceFromPancakeswap(this.SFI);
    const priceOfOneETH = await this.getWETHPriceFromPancakeswap();
    const priceOfSfiInDollars = (Number(priceInETH) * Number(priceOfOneETH)).toFixed(2);

    return {
      tokenInEth: priceInETH,
      priceInDollars: priceOfSfiInDollars,
      totalSupply: getDisplayBalance(supply, this.SFI.decimal, 0),
      circulatingSupply: getDisplayBalance(sfiCirculatingSupply, this.SFI.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('SFI') ? this.SFI : this.TSHARE;
    const isSfi = name.startsWith('SFI');
    ...
    const lpTokenPrice = await this.getLPTokenPrice(lpToken, token0, isSfi);
  ...
  /**
   * Use this method to get price for Sfi
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const sfiStat = await this.getSfiStat();
    const bondSfiRatio = await Treasury.getBondPremiumRate();
    let modifier = 1; // keep to 1 if no bondPremium is to be used
    if (getBalance(bondSfiRatio, this.SFI.decimal) > 0) {
      modifier = getBalance(bondSfiRatio, this.SFI.decimal);
    }
    const bondPriceInETH = (Number(sfiStat.tokenInEth) * modifier).toFixed(2);
    const priceOfTBondInDollars = (Number(sfiStat.priceInDollars) * modifier).toFixed(2);
  ...
  async getShareStat(): Promise<TokenStat> {
    const { SfiEthLPTShareRewardPool } = this.contracts;

    const supply = await this.TSHARE.totalSupply();

    const priceInETH = await this.getTokenPriceFromPancakeswap(this.TSHARE);
    const sfiRewardPoolSupply = await this.TSHARE.balanceOf(SfiEthLPTShareRewardPool.address);
    const tShareCirculatingSupply = supply.sub(sfiRewardPoolSupply);
    const priceOfOneETH = await this.getWETHPriceFromPancakeswap();
  ...
  async getSfiStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, SfiEthRewardPool } = this.contracts;
    const expectedPrice = await SeigniorageOracle.twap(this.SFI.address, ethers.utils.parseEther('1'));

    const supply = await this.SFI.totalSupply();
    const sfiRewardPoolSupply = await this.SFI.balanceOf(SfiEthRewardPool.address);
    const sfiCirculatingSupply = supply.sub(sfiRewardPoolSupply);
    return {
  ...
  async getSfiPriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getSfiUpdatedPrice();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'SFI' ? await this.getSfiStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'SFI') {
      if (!contractName.endsWith('SfiRewardPool')) {
        const rewardPerSecond = await poolContract.sfiPerSecond();
      ...
      if (Date.now() - startDateTime.getTime() > FOUR_DAYS) {
        return await poolContract.epochSfiPerSecond(1);
      }
      return await poolContract.epochSfiPerSecond(0);
    }
    const rewardPerSecond = await poolContract.tSharePerSecond();
    if (depositTokenName.startsWith('SFI')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WETH') {
      ...
      if (tokenName === 'SFI-ETH-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.SFI, true);
      } else if (tokenName === 'TSHARE-ETH-LP') {
  ...
  async buyBonds(amount: string | number): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const treasurySfiPrice = await Treasury.getSfiPrice();
    return await Treasury.buyBonds(decimalToBalance(amount), treasurySfiPrice);
  }
  ...
  async redeemBonds(amount: string): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const priceForSfi = await Treasury.getSfiPrice();
    return await Treasury.redeemBonds(decimalToBalance(amount), priceForSfi);
  }
   ...
   * @param isSfi sanity check for usage of sfi token or tShare
   * @returns price of the LP token
   */
  async getLPTokenPrice(lpToken: ERC20, token: ERC20, isSfi: boolean): Promise<string> {
    const totalSupply = getFullDisplayBalance(await lpToken.totalSupply(), lpToken.decimal);
    //Get amount of tokenA
    const tokenSupply = getFullDisplayBalance(await token.balanceOf(lpToken.address), token.decimal);
    const stat = isSfi === true ? await this.getSfiStat() : await this.getShareStat();
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'SFI') {
        return await pool.pendingSFI(poolId, account);
      } else {
  ...
  async getTokenPriceFromPancakeswap(tokenContract: ERC20): Promise<string> {
    ...
    const weth = new Token(chainId, WETH[0], WETH[1]);
    const token = new Token(chainId, tokenContract.address, tokenContract.decimal, tokenContract.symbol);
    try {
      const wethToToken = await Fetcher.fetchPairData(weth, token, this.provider);
      const priceInBUSD = new Route([wethToToken], token);
  ...
  async getWETHPriceFromPancakeswap(): Promise<string> {
    const ready = await this.provider.ready;
    if (!ready) return;
    const { WETH, LOOKS } = this.externalTokens;
    try {
      const looks_weth_lp_pair = this.externalTokens['USDT-ETH-LP'];
      let eth_amount_BN = await WETH.balanceOf(looks_weth_lp_pair.address);
      let eth_amount = Number(getFullDisplayBalance(eth_amount_BN, WETH.decimal));
      let looks_amount_BN = await LOOKS.balanceOf(looks_weth_lp_pair.address);
      let looks_amount = Number(getFullDisplayBalance(looks_amount_BN, LOOKS.decimal));
      return (looks_amount / eth_amount).toString();
    } catch (err) {
  ...
  async getMasonryAPR() {
    ...
    const TSHAREPrice = (await this.getShareStat()).priceInDollars;
    const SFIPrice = (await this.getSfiStat()).priceInDollars;
    const epochRewardsPerShare = lastRewardsReceived / 1e18;

    //Mgod formula
    const amountOfRewardsPerDay = epochRewardsPerShare * Number(SFIPrice) * 4;
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      ...
      if (assetName === 'SFI') {
        asset = this.SFI;
        assetUrl = 'https://sfi.finance/presskit/sfi_icon_noBG.png';
      } else if (assetName === 'TSHARE') {
        asset = this.TSHARE;
        assetUrl = 'https://sfi.finance/presskit/tshare_icon_noBG.png';
      } else if (assetName === 'TBOND') {
        asset = this.TBOND;
        assetUrl = 'https://sfi.finance/presskit/tbond_icon_noBG.png';
      }
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === ETH_TICKER) {
      ...
      const token = tokenName === SFI_TICKER ? this.SFI : this.TSHARE;
